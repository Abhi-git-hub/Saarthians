...
export async function startTestV2(testId: string) {
  ...
  const { data: attempt } = await supabase.from("test_attempts").select("id,started_at").eq("id", data as string).single();
  return { attemptId: data as string, startedAt: attempt?.started_at ?? new Date().toISOString() };
}
...
export async function submitTestV2(attemptId: string, testId: string) {
  await requireRole(["student"]);
