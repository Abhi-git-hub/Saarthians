  ...
  return (
    <main className="container" style={{ padding: "48px 0 80px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 18, alignItems: "end", flexWrap: "wrap" }}>
        <div><span className="eyebrow">Learning material</span><h1 style={{ fontSize: "clamp(40px,6vw,68px)", lineHeight: .95, letterSpacing: "-.06em", margin: "16px 0 12px" }}>Teach it once.</h1><p style={{ color: "var(--muted)", maxWidth: 620, lineHeight: 1.7, margin: 0 }}>Write it once — every student and teacher reads it. Attach pictures where words fall short.</p></div>
      ...
      <form method="get" style={{ marginTop: 30, display: "flex", gap: 10 }}>
        <input name="q" defaultValue={search} maxLength={120} placeholder="Search all material…" aria-label="Search material" style={inputStyle} />
            ...
            <article key={note.id} style={{ borderTop: "1px solid var(--line)", padding: "20px 0", display: "grid", gridTemplateColumns: "1fr auto", gap: 22, alignItems: "start" }}>
              <div>
                <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                  <Link href={`/teacher/notes/${note.id}`} style={{ fontSize: 22, fontWeight: 750, letterSpacing: "-.03em" }}>{note.title}</Link>
                  <span style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: ".1em", color: "var(--muted)" }}>by {own ? "you" : note.owner_name}</span>
                </div>
                <p style={{ color: "var(--muted)", lineHeight: 1.6, margin: "7px 0 0", maxWidth: 760 }}>{note.content.slice(0, 180) || "Empty note"}</p>
              </div>
              {own && (
