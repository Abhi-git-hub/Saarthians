  ...
  return (
    <main className="container" style={{ padding: "48px 0 80px", maxWidth: 920 }}>
      ...
      <div style={{ marginTop: 24 }}>
        ...
        <p style={{ color: "var(--muted)", maxWidth: 640, lineHeight: 1.65, margin: "0 0 34px" }}>Administrator workspace preferences are intentionally minimal: your own credentials, nothing else to misconfigure.</p>
      </div>
      <div style={{ display: "grid", gap: 14 }}>
        <section style={cardStyle}>
          <span className="eyebrow">Account</span>
          <div style={{ display: "grid", gap: 0, marginTop: 8 }}>
            {[["Name", user.displayName], ["Username", user.username ? `@${user.username}` : "—"], ["Role", "Administrator"], ["Access", "Active"]].map(([label, value]) => (
        ...
        <section style={cardStyle}>
          ...
          <p style={{ color: "var(--muted)", fontSize: 14, margin: "0 0 20px" }}>Signing out ends this session on this device. Locked out? <Link href="/login/reset-password" style={{ textDecoration: "underline" }}>Reset your password by email</Link>.</p>
          <SignOutButton />
        </section>
      </div>
    </main>
